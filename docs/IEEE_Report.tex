...
\title{Performance Optimization of a Pipelined MIPS Processor: A Comprehensive Analysis with 13 Contributions}
...
\IEEEPARstart{M}{odern} processor design relies on multiple optimization techniques to bridge the CPU-memory gap---the ``Memory Wall'' problem~\cite{hennessy2017computer}. This project implements 13 optimizations for a 5-stage MIPS pipeline. This section provides foundational background for readers unfamiliar with computer architecture concepts.
...
This project implements 13 distinct optimizations, summarized in Table~\ref{tab:contributions}. Each optimization targets a specific bottleneck, and their combined effect is analyzed using Design of Experiments (DOE) methodology.

\begin{table}[!t]
\centering
\caption{13 Optimization Contributions by Category}
...
\midrule
Parser Enhancement & \#8 Expression Parser \\
\midrule
Calculus Operations & \#13 ODE Solver (Euler Method) \\
\bottomrule
\end{tabular}
\end{table}
...
This section describes each of the 13 contributions in detail, with implementation specifics and verification results.

\subsection{Contribution 1 \& 4: Data Forwarding and CPI Analysis}
...
\textbf{DOE Variable Implementation Specifications}:
\begin{table}[!t]
\centering
\caption{Implementation Specifications for DOE Variables}
\label{tab:doespec}
\footnotesize
\begin{tabular}{@{}llp{4.2cm}@{}}
\toprule
\textbf{Abbrev} & \textbf{Implementation Type} & \textbf{Key Parameters} \\
\midrule
FWD & Full Forwarding & EX/MEM $\to$ EX, MEM/WB $\to$ EX \\
BP & 2-bit Saturating Counter (PC-indexed) & 16-entry BHT, 2-bit counters, PC[5:2], \textbf{78.33\% accuracy} \\
Cache & L1 Only, Direct-Mapped, Write-Back & 8KB, 32B block, 256 lines, \textbf{97.22\% hit rate} \\
SIMD & Packed SIMD (8$\times$8-bit) & 5 ops (+, $-$, $\times$, $\div$, \^{}) \\
\bottomrule
\end{tabular}
\end{table}

\textit{Note: BP accuracy is weighted average across 4 patterns (Always Taken 90\%, Always Not Taken 85\%, Alternating 73\%, Loop 78\%). Cache uses Write-Allocate policy.}
...
\textbf{Implementation Notes}: The Shunting-yard algorithm requires two stack structures: an operator stack for temporary storage and an output queue for the postfix result. Our hardware implementation uses a 32-entry operator stack and processes one token per clock cycle. Right associativity for exponentiation is handled by comparing precedence with strict inequality when the incoming operator is right-associative.
...
\subsubsection{Natural Logarithm: ln(x)}
To complement $\exp(x)$, we also implement its inverse function $\ln(x)$ using the same range reduction approach.

\textbf{Algorithm}:
\begin{equation}
\ln(x) = \ln(2^k \cdot m) = k \cdot \ln(2) + \ln(m)
\end{equation}
where $k$ is determined via leading-zero detection, and $m \in [1, 2)$.

For $\ln(m)$, we use $y = m - 1 \in [0, 1)$ and approximate:
\begin{equation}
...
\begin{table}[!t]
\centering
\caption{Polynomial ln(x) Test Results}
\footnotesize
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{Input $x$} & \textbf{Expected} & \textbf{Result} & \textbf{Error} & \textbf{Status} \\
...
%-----------------------------------------------------------------------------
\subsection{Contribution 13: ODE Solver (Calculus Operations)}

\textbf{Goal}: Implement a hardware Ordinary Differential Equation (ODE) solver using the Forward Euler method, demonstrating calculus operations (differentiation and integration) in hardware.
...
Where:
\begin{itemize}
    \item $X$ is the state vector (size $N$)
    \item $A$ is the system matrix ($N \times N$)
    \item $B$ is the input matrix ($N \times M$)
    \item $U$ is the input vector (size $M$)
\end{itemize}
...
\subsubsection{Why More Complex Than CORDIC}
\begin{table}[!t]
\centering
\caption{ODE Solver vs CORDIC Complexity Comparison}
...
The ODE solver consists of:
\begin{itemize}
    \item \textbf{Euler.v}: Main solver module (279 lines) with 6-state FSM
    \item \textbf{RAM-Pre.v}: Dual-port RAM for matrices and vectors
...
\begin{enumerate}
    \item \textbf{Start}: Initialize, read $N$, $M$ from RAM
    \item \textbf{Prepare}: Load system parameters
    \item \textbf{Interpolate}: Read step size $h$
    \item \textbf{Load1}: Compute $B \cdot U$ (matrix-vector multiplication)
    \item \textbf{Load2}: Compute $A \cdot X$ (matrix-vector multiplication)
    \item \textbf{Final\_Calc}: Compute $X_{new} = X + h \cdot (A \cdot X + B \cdot U)$
\end{enumerate}

\textbf{Source Attribution}: Based on 3amrA7med/ODE-Solver (GitHub), adapted for integration with MIPS processor project.

\subsubsection{Verified Test Results}
\textbf{TCL Command}:
\begin{lstlisting}
...
\textbf{Results}:
\begin{itemize}
    \item $X_{new}[0] = 25$ (Expected: 25) --- \textbf{PASS}
    \item $X_{new}[1] = 45$ (Expected: 45) --- \textbf{PASS}
\end{itemize}
...
This section consolidates the results from all 13 contributions and provides interpretation guidance.
