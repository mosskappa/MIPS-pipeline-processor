...
\textbf{Solution}: Cache memory hierarchy. By keeping frequently accessed data in small, fast caches (L1, L2, L3), processors can avoid slow main memory accesses most of the time. Our L1 cache implementation reduces average memory access time by \textbf{16.7$\times$}.
...
\begin{verbatim}
Sequential (8 cycles):       SIMD 8 lanes (1 cycle):
  a[0] + b[0] -> y[0]          a[0..7] + b[0..7]
  a[1] + b[1] -> y[1]                 |
  a[2] + b[2] -> y[2]                 v
  ...                            y[0..7]
  a[7] + b[7] -> y[7]          (all in 1 cycle!)
\end{verbatim}
...
L1 Cache & AMAT & 100$\rightarrow$6 (16.7$\times$) & Memory Wall \\
...
\item \textbf{Cache Memory} provides the largest individual improvement: 16.7$\times$ AMAT reduction for L1 alone, up to 54.8$\times$ with three-level hierarchy.
